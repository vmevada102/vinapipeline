...
def ligand_network(receptor, max_ligands=100, top_edges=300):
    ...
    print(f"[NETWORK] Processing {receptor}")

    f = FINAL_DIR / f"{receptor}_ifp_matrix.csv"
    if not f.exists():
        print(f"[SKIP NETWORK] No IFP for {receptor}")
        return

    df = pd.read_csv(f, index_col=0)

    # ?? limit ligands (performance)
    ...
    # -------------------------------
    # ?? LABEL CONTROL (important)
    # -------------------------------
    # Show only top connected nodes (avoid clutter)
    ...
    # optional: edge labels (can be heavy)
...
def ligand_top10_network(receptor):
    ...
    # ?? limit comparisons (speed)
    max_check = min(len(lig), 50)
    ...
    # optional edge labels (similarity)
    edge_labels = {(a, b): f"{d['weight']:.2f}" for a, b, d in G.edges(data=True)}
    nx.draw_networkx_edge_labels(G, pos, edge_labels=edge_labels, font_size=6)

    plt.title(f"{receptor} Top-10 Ligand Network")
    plt.axis("off")
