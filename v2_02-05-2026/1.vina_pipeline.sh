...
if df.empty:
    print("[WARNING] No PLIP interactions found for Top-N ligands")
    print("[INFO] Falling back to ALL ligands with interactions")

    df = pd.read_csv("results/interactions/plip_interactions_all.csv")

    if df.empty:
        print("[ERROR] No PLIP interactions available at all")
        SKIP_PCA = True
...
if fp.empty or fp.shape[0] == 0 or fp.shape[1] == 0:
    print("[WARNING] Empty interaction fingerprint matrix. Skipping PCA & clustering.")
    SKIP_PCA = True
...
if fp.shape[0] < 2:
    print("[WARNING] Not enough ligands for PCA (need at least 2). Skipping.")
    SKIP_PCA = True
...
if not SKIP_PCA:
    ...
    plt.savefig("results/interactions/figures/interaction_pca_clustering.png")
    plt.close()

    print("Interaction-type PCA clustering completed")

else:
    print("[INFO] PCA skipped safely")
...
if prolif_rows:
    pd.DataFrame(
        prolif_rows,
        columns=["receptor", "ligand", "interaction_type", "residue"]
    ).to_csv(
...
# ==================================================
# FINAL PLIP PARSING (ALL COMPLEXES)
# ==================================================
...
for receptor_dir in PLIP_BASE.iterdir():
    ...
    for ligand_dir in receptor_dir.iterdir():
        ...
        # ? THIS MUST BE INSIDE ligand loop
        for block, interaction_name in PLIP_BLOCKS.items():
            ...
            for item in block_node:
                ...
                distance = item.findtext("distance", default="NA")
                angle = item.findtext("angle", default="NA")
...
if rows:
    ...
    # ? CORRECT indentation (same level as df = ...)
...
fingerprint.to_csv(
    ...
    # TOP-N
    try:
        top_ligands = pd.read_csv(
            "results/summary/ligand_consensus_ranking.csv",
            index_col=0
        ).head(int(os.getenv("TOP_N_ANALYSIS", "20"))).index

        df_top = df[df["ligand"].isin(top_ligands)]
        df_top.to_csv(outdir / "plip_interactions_topN.csv", index=False)

    except Exception as e:
        print("[WARNING]", e)

else:
    print("[WARNING] No PLIP interactions found")
