...
# global top ligands (mean energy)
top_ligands = (
    df.groupby("ligand")["energy"]
      .mean()
      .sort_values()
...
print("Top-N folders created for receptors:")
for d in TOP_DIR.iterdir():
    if d.is_dir():
        print("  -", d.name, ":", len(list(d.glob("*_best.pdbqt"))), "ligands")
...
GNINA_AVAILABLE = GNINA_PATH.exists()

if GNINA_AVAILABLE:
    GNINA_CMD = str(GNINA_PATH.resolve())
else:
    GNINA_CMD = "gnina"

if GNINA_AVAILABLE:
    print("GNINA detected: enabling re-scoring for TOP ligands")
else:
    print("GNINA not found: skipping re-scoring step")
...
top_ligands = (
    pd.read_csv("results/summary/ligand_consensus_ranking.csv", index_col=0)
      .head(TOP_N)
      .index
)

df = df[df["ligand"].isin(top_ligands)]
...
#===================================================="
# FULL COMPLEX GENERATION (ALL LIGANDS)
#===================================================="
from pathlib import Path
import subprocess
import shutil
...
ALL_COMPLEX_DIR.mkdir(parents=True, exist_ok=True)
...
print("Generating complexes for ALL ligands...")

for receptor in df["receptor"].unique():
    dock_dir = Path("results/docking") / receptor
    receptor_pdbqt = Path("receptors") / f"{receptor}.pdbqt"

    rec_out = ALL_COMPLEX_DIR / receptor
    rec_out.mkdir(exist_ok=True)

    for lig_file in dock_dir.glob("*_out.pdbqt"):
        ligand = lig_file.stem.replace("_out", "")

        temp_best = rec_out / f"{ligand}_best_tmp.pdbqt"
        lig_pdb = rec_out / f"{ligand}.pdb"
        complex_pdb = rec_out / f"{ligand}_complex.pdb"

        extract_best_pose(lig_file, temp_best)
        pdbqt_to_pdb(temp_best, lig_pdb)
        make_complex(receptor_pdbqt, lig_pdb, complex_pdb)

        temp_best.unlink()

print("All complexes generated.")


#===================================================="
# PARALLEL PLIP EXECUTION (FAST ??)
#===================================================="
...
def run_plip_job(args):
    complex_pdb, receptor = args
    ligand = complex_pdb.stem.replace("_complex", "")
...
if PLIP_AVAILABLE:
    ...
    print("Running PLIP in PARALLEL...")

    jobs = []
    for complex_pdb in ALL_COMPLEX_DIR.rglob("*_complex.pdb"):
        receptor = complex_pdb.parent.name
        jobs.append((complex_pdb, receptor))

    cpu = max(1, mp.cpu_count() - 2)

    print(f"Total PLIP jobs: {len(jobs)}")
    print(f"Using CPUs: {cpu}")

    with mp.Pool(cpu) as pool:
        pool.map(run_plip_job, jobs)

    print("PLIP parallel execution completed.")

else:
    print("PLIP not found — skipping interaction analysis")


#===================================================="
# GNINA RESCORING (TOP-N ONLY)
#===================================================="
...
GNINA_OUT = Path("results/gnina")
GNINA_OUT.mkdir(parents=True, exist_ok=True)

def run_gnina_job(args):
    receptor, ligand_file = args

    ligand = ligand_file.stem.replace("_best", "")
    receptor_pdbqt = Path("receptors") / f"{receptor}.pdbqt"

    out_file = GNINA_OUT / f"{receptor}__{ligand}.pdbqt"
    log_file = GNINA_OUT / f"{receptor}__{ligand}.log"
...
if GNINA_AVAILABLE:
    ...
    print("Running GNINA rescoring on TOP ligands...")

    jobs = []

    for receptor_dir in Path("results/top_N").iterdir():
        if not receptor_dir.is_dir():
            continue

        receptor = receptor_dir.name

        for lig_file in receptor_dir.glob("*_best.pdbqt"):
            jobs.append((receptor, lig_file))

    cpu = max(1, mp.cpu_count() - 2)

    with mp.Pool(cpu) as pool:
        pool.map(run_gnina_job, jobs)

    print("GNINA rescoring completed")

else:
    print("Skipping GNINA step")
...
pattern = re.compile(r"CNNscore\s*=\s*(-?\d+\.\d+)")

for log in GNINA_OUT.glob("*.log"):

    name = log.stem
    receptor, ligand = name.split("__")
    ...
    if score is not None:
        rows.append([receptor, ligand, score])

if rows:
    df_gnina = pd.DataFrame(
        rows,
        columns=["receptor", "ligand", "cnn_score"]
    )

    df_gnina.to_csv("results/summary/gnina_rescoring.csv", index=False)

    print("GNINA scores saved")
else:
    print("No GNINA scores detected")


#===================================================="
# HYBRID SCORING (VINA + GNINA)
#===================================================="

vina_df = pd.read_csv("results/summary/vina_best_scores.csv")

if "df_gnina" in locals():
    ...
    merged["gnina_norm"] = (
        merged["cnn_score"] - merged["cnn_score"].mean()
    ) / merged["cnn_score"].std()

    # Hybrid score (adjust weights if needed)
    merged["hybrid_score"] = (
        -1 * merged["vina_norm"] + merged["gnina_norm"]
    )

    merged.to_csv(
        "results/summary/hybrid_scoring.csv",
        index=False
    )

    print("Hybrid scoring generated")
